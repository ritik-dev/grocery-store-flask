...
from GroceryStore.makegraph import makegraph,calculate_total_revenue_till_now, calculate_total_revenue_this_month
from pathlib import Path
from GroceryStore.forms import RegistrationForm, LoginForm, AdminLoginForm, CategoryForm, ProductForm, ProductSearchForm
from GroceryStore.models import User, Category, Product, Orders
...
UPLOAD_FOLDER = 'GroceryStore\static'
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg'}

UPLOAD_FOLDER = os.path.join(os.getcwd(), 'GroceryStore', 'static')
...
def manage_products():
    ...
    if form.validate_on_submit():
        ...
        if form.image.data and allowed_file(form.image.data.filename):
            ...
            filename = f"{new_product.id}.{secure_filename(image.filename.split('.')[-1])}"
...
def buy_success():
    ...
    user = current_user
    max_order = db.session.query(db.func.max(Orders.order_id)).scalar()

    # If there are no orders yet, set max_order to 0, otherwise, increment it by 1
    ...
    # Update the purchases table for each product bought
    for cart_item in current_user.purchases:
        if cart_item.quantity > 0:
            purchase = Orders(
                ...
                price_of_item=cart_item.rate_per_unit,
...
def stats():
    ...
    makegraph()
    if not chart_filename.is_file():
        # If the chart doesn't exist, generate it
        makegraph()
        return render_template("stats.html", loading=True, title='Statistics')

     # Calculate total revenue till now
    total_revenue_till_now = calculate_total_revenue_till_now()

    # Calculate total revenue of this month
    total_revenue_this_month = calculate_total_revenue_this_month()

    # Create a string variable with the combined sentences
    combined_sentence = f"Total Revenue Till Now: Rs. {total_revenue_till_now}<br>Total Revenue This Month: Rs. {total_revenue_this_month}"

    # Pass the combined sentence variable to the template context
    return render_template("stats.html", loading=False, title='Statistics', combined_sentence=combined_sentence)
...
@login_required
def reset_stats():
    # Call the makegraph() function to regenerate the chart
    makegraph()
    flash('Statistics reset successfully!', 'success')
